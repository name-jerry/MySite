	...
	<view class="search-wrap">
		<view class="search-box hover-shadow-input" :class="{'blank':isBlank}">
			<input type="text" class="search" confirm-type="search" @confirm="enter" v-model='searchWord' @blur="blur"
				@focus="focusInput" />
			<Options class="options" :class="{show:showOptions}" :options='optionList' :show-mask='false'
				@select='select'>
			</Options>
			...
			<a ref='btn' :href='toolList[index].value+searchWord' @tap='search' target='_blank'
				class="btn hover-shadow-btn">
...
<script setup lang='ts'>
	...
	let toolList = reactive < {
		...
		{
			label: 'Can I Use',
			value: 'https://caniuse.com/?search='
		},
		{
			label: '菜鸟教学',
			value: 'https://www.runoob.com/?s='
		},
	])
