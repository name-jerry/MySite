<template>
	<div class="markdown-box" :class="{coexist:isCoexist}">
		...
		<div ref='markdownBody' class="markdown-body" v-show="!prop.showMd||isCoexist" v-html="mdHtml"
			@click="clickToc">
		</div>
...
<script setup lang="ts">
	...
	onMounted(() => {
		...
		/**动态设置目录位置 */
		watch(tocShow, () => {
			const tocEl = document.getElementsByClassName('table-of-contents')[0] as HTMLElement;
			const head = (document.getElementsByClassName('markdown-head')[0] as HTMLElement)
			tocEl.style.top = head.offsetTop - 10 + 'px';
			document.body.style.overflow = tocShow.value ? 'hidden' : 'auto';
			if (tocShow.value) {
				tocEl.style.maxHeight = document.body.clientHeight - head.clientHeight + "px"
			}
